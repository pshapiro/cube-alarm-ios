...
def get_cube_status():
    ...
    return jsonify({
        'connected': cube_worker.is_connected() if cube_worker else False,
        'solved': cube_worker.is_solved() if cube_worker else False
    })
...
def run_cube_worker():
    ...
    # Set up event callbacks directly
    ...
    # Run cube worker
    try:
        cube_worker.run_sync()
    except Exception as e:
        logger.error(f"Cube worker error: {e}")
