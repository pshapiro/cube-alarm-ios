#!/usr/bin/env python3
"""
Test cube connection using the REAL MAC address we discovered.
This should finally work with proper decryption and move detection!
"""
...
# REAL MAC ADDRESS discovered from BLE manufacturer data
...
async def notification_handler(_, data: bytes) -> None:
    """Notification handler with REAL MAC address decryption."""
    global _key_iv
...
async def find_and_connect_cube():
    """Find and connect to GAN cube using REAL MAC address."""
    global _key_iv
    ...
    _log(f"🔑 Using REAL MAC address: {REAL_MAC_ADDRESS}")
    
    # Derive encryption key using the REAL MAC ADDRESS
    try:
        from gan_decrypt import derive_key_iv
        _key_iv = derive_key_iv(REAL_MAC_ADDRESS)  # Use real MAC, not UUID!
        _log(f"🔑 Key derived with real MAC: {_key_iv[0].hex()[:16]}...")
        _log(f"🔑 IV derived with real MAC: {_key_iv[1].hex()[:16]}...")
    except Exception as e:
    ...
    try:
        ...
        _log("📡 Notifications started with REAL MAC decryption - move your cube!")
        _log("🎯 This should finally show proper Gen3 move detection!")
...
def main():
    """Main function."""
    print("🎉 FINAL TEST: Real MAC Address Move Detector")
    print("=" * 60)
    print("Using the REAL MAC address extracted from BLE manufacturer data:")
    print(f"Real MAC: {REAL_MAC_ADDRESS}")
    print("This matches the working JavaScript implementation exactly!")
    print()
    print("Expected results:")
    print("✅ Gen3 magic byte (0x55) in decrypted packets")
    print("✅ Proper move detection (U, R, F, D, L, B with ' for prime)")
    print("✅ Serial numbers and timestamps")
    print()
    print("Move your cube to test REAL move detection!")
